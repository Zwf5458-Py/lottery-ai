...
def _probe_openai_endpoint(api_base, model, api_key, timeout=15):
    ...
    try:
        ...
        return {'ok': True, 'status': 'green', 'message': '连通成功', 'url': url, 'preview': body[:300]}
...
def _build_ai_diagnosis_data():
    from modules.config_manager import load_global_config, get_platform_owner_ai_config, get_platform_owner_identity

    global_ai = (load_global_config() or {}).get('ai', {})
    owner_ai = get_platform_owner_ai_config() or {}
    owner = get_platform_owner_identity() or {}

    effective_base = owner_ai.get('api_base') or global_ai.get('api_base') or os.environ.get('LOCAL_AI_BASE', '') or os.environ.get('HOST_GATEWAY_URL', '')
    effective_model = owner_ai.get('model') or global_ai.get('model') or 'gpt-5.4'
    effective_key = owner_ai.get('api_key') or global_ai.get('api_key') or os.environ.get('LOCAL_AI_API_KEY', '')

    direct_probe = _probe_openai_endpoint(effective_base, effective_model, effective_key)
    ...
    docker_probe = {
        ...
        'status': 'green' if docker_probe_raw.get('ok') else ('yellow' if str(docker_probe_raw.get('error', '')).startswith('HTTP 4') else 'red'),
        'message': '容器连通成功' if docker_probe_raw.get('ok') else (docker_probe_raw.get('error') or '未执行'),
    ...
    statuses = {
        ...
        'env_snapshot': {
            'status': 'green' if (docker_env.get('LOCAL_AI_BASE') or docker_env.get('HOST_GATEWAY_URL')) else 'yellow',
            'message': '已获取环境快照',
    ...
    return {
        ...
        'autofix_preview': {
            ...
            'will_change': bool(recommended_base and recommended_base != (owner_ai.get('api_base') or effective_base)),
            'reason': recommendation_reason,
            'source': 'docker_probe' if docker_probe.get('ok') else ('direct_probe' if direct_probe.get('ok') else 'fallback'),
        },
    }
...
from modules.auth import (
    ...
    get_user_db_connection, USERS_DB_PATH
)
...
def healthz():
    ...
    return jsonify({'status': status, 'service': 'lottery_app'}), 200 if cache_ok else 503
...
def api_simulate():
    ...
    try:
        ...
        dimensions = data.get('dimensions', ['big_small', 'odd_even', 'hot_cold', 'tail', 'markov'])
        ...
        if mode in ('ai', 'ai_zodiac') and session.get('role') != 'admin':
            ...
            if ai_cost > 0:
                ...
                if not deduct_res.get('success'):
                    return jsonify({
                        'success': False,
                        'error': deduct_res.get('error', '积分不足，无法进行 AI 模拟'),
...
def api_save_settings():
    ...
    try:
        ...
        current_periods = current_cfg.get('chart_periods', {}) if isinstance(current_cfg, dict) else {}
        incoming_periods = data.get('chart_periods', {}) if isinstance(data.get('chart_periods', {}), dict) else {}
...
def api_account_points_center():
    """积分中心聚合数据"""
    try:
        from modules.points_manager import get_user_points, get_points_ledger, get_recharge_history
        ...
        if not ledger_res.get('success'):
            return jsonify({'success': False, 'error': ledger_res.get('error', '积分流水读取失败')}), 500
        if not recharge_res.get('success'):
            return jsonify({'success': False, 'error': recharge_res.get('error', '充值记录读取失败')}), 500
...
def api_admin_system_config():
    ...
    payload = {
        'system': {
            ...
            'settings_change_cost': max(0, int(data.get('settings_change_cost', 1) or 0)),
            'share_reward_ratio': max(0.0, float(data.get('share_reward_ratio', 0.2) or 0.0)),
            'share_reward_points': max(0, int(data.get('share_reward_points', 10) or 0)),
            'share_recharge_min': max(0.0, float(data.get('share_recharge_min', 5) or 0.0)),
    ...
    cache.clear()
    return jsonify({'success': True, 'message': '系统配置已保存', 'data': get_system_config()})
...
def api_admin_ai_config():
    ...
    effective = {
        ...
        'api_base': owner_ai.get('api_base') or global_ai.get('api_base') or os.environ.get('LOCAL_AI_BASE', ''),
        'api_key_masked': _mask_secret(owner_ai.get('api_key') or global_ai.get('api_key') or os.environ.get('LOCAL_AI_API_KEY', '')),
    }
...
def api_admin_ai_config_save():
    ...
    return jsonify({'success': True, 'message': '平台管理员 AI 配置已保存', 'data': ai_payload})
...
def api_admin_ai_config_test():
    ...
    api_key = owner_ai.get('api_key') or global_ai.get('api_key') or os.environ.get('LOCAL_AI_API_KEY', '')
    ...
    return jsonify({
        ...
        'data': {'url': result.get('url', ''), 'model': model, 'preview': result.get('preview', ''), 'detail': result.get('detail', '')}
    }), status_code
...
def api_admin_ai_effective_user():
    ...
    from modules.config_manager import load_config, load_global_config, get_platform_owner_ai_config, get_platform_owner_identity
    ...
    effective = {
        'platform': user_cfg.get('platform') or owner_ai.get('platform') or global_ai.get('platform', ''),
        'model': user_cfg.get('model') or owner_ai.get('model') or global_ai.get('model', ''),
        'api_base': user_cfg.get('api_base') or owner_ai.get('api_base') or global_ai.get('api_base', '') or os.environ.get('LOCAL_AI_BASE', ''),
        'api_key_source': 'user' if user_cfg.get('api_key') else ('platform_owner' if owner_ai.get('api_key') else ('global_or_env' if (global_ai.get('api_key') or os.environ.get('LOCAL_AI_API_KEY')) else 'none'))
    }
...
def api_admin_ai_config_test_docker():
    ...
    container_name = str(container_name.get('container', 'macau-mark-six')).strip() or 'macau-mark-six'
    parsed = _run_docker_ai_probe(container_name)
    return jsonify({'success': bool(parsed.get('ok')), 'data': parsed}), 200 if parsed.get('ok') else 400
...
def api_admin_ai_config_docker_env():
    ...
    container_name = request.args.get('container', 'macau-mark-six').strip() or 'macau-mark-six'
    data = _get_docker_ai_env_snapshot(container_name)
    if not data.get('ok'):
        return jsonify({'success': False, 'error': data.get('error', '读取失败'), 'data': data}), 400
...
def api_admin_ai_config_compare():
    ...
    consistency = {
        ...
        'base_consistent': len({x for x in [effective_base, owner_ai.get('api_base', ''), global_ai.get('api_base', ''), env_base, docker_base] if x}) <= 1,
    }
...
def api_admin_ai_config_apply_tested_base():
    ...
    if not ok:
        return jsonify({'success': False, **payload_meta}), 404
    return jsonify({'success': True, 'message': '已写入平台默认地址', 'data': current_ai})
...
def api_admin_ai_config_test_and_auto_fix():
    ...
    container_name = str(payload.get('container', 'macau-mark-six') or 'macau-mark-six').strip() or 'macau-mark-six'
...
def api_admin_ai_config_recent_failures():
    ...
    keywords = ['AI 分析异常', '所有候选模型均失败', 'HTTP 401', 'HTTP 404', 'URLError', 'Connection refused']
    try:
...
def api_admin_recharge():
    ...
    reward_points = int(round(base_points * reward_ratio)) if reward_ratio > 0 else reward_fixed
    if reward_points <= 0:
    ...
    reward_res = apply_referral_recharge_reward(user_id, amount, recharge_min, reward_points)
    return jsonify({
        'success': True,
        'message': '充值成功',
        'data': {
...
def api_admin_health():
    ...
    try:
        ...
        health['data_sync'] = {'status': 'green' if days <= 1 else ('yellow' if days <= 3 else 'red'), 'message': f'数据滞后 {days} 天'}
    except Exception as e:
    ...
    try:
        ...
        global_ai = (load_global_config() or {}).get('ai', {})
        api_base = owner_ai.get('api_base') or global_ai.get('api_base') or os.environ.get('LOCAL_AI_BASE', '')
        api_key_ok = bool(owner_ai.get('api_key') or global_ai.get('api_key') or os.environ.get('LOCAL_AI_API_KEY', ''))
        health['ai'] = {'status': 'green' if api_base and api_key_ok else 'yellow', 'message': f"API Base={api_base or '-'} ｜ Key={'已配置' if api_key_ok else '未配置'}"}
    except Exception as e:
    ...
    try:
        ...
        health['cache'] = {'status': 'green' if val == 'ok' else 'yellow', 'message': '缓存可用' if val == 'ok' else '缓存读写异常'}
    except Exception as e:
    ...
    try:
        ...
        health['logs'] = {'status': 'green' if exists else 'yellow', 'message': f"AI 故障日志 {'存在' if exists else '未生成'} ｜ {size} bytes"}
    except Exception as e:
...
if __name__ == '__main__':
    ...
    if not os.environ.get('FLASK_SECRET_KEY'):
        logger.warning("FLASK_SECRET_KEY 未设置，当前使用进程内临时密钥（重启后会失效）。")
