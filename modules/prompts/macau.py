...
def _build_macau_analysis_prompt(stats: dict, lottery_type: str, dimensions: list, pre_sel_nums: list = None, pre_sel_special: int = 1, system_weights: dict = None, is_wheeling: bool = False) -> str:
    ...
    if 'three_region' in dimensions: conv_parts.append('三分比走势')
    if 'prime_composite' in dimensions: conv_parts.append('质合比走势')
    if 'repeats_tails' in dimensions: conv_parts.append('重号同尾分布')
    if 'ac_value' in dimensions: conv_parts.append('AC值复杂度')
    if 'range_span' in dimensions: conv_parts.append('极大极距走势')
    if 'mean_regression' in dimensions: conv_parts.append('均值离差回归')
    ...
    report_parts.append(f"2. **加权维度逐项剖析**：【⚠️重要纪律要求：必须依次输出下方这 {len(dimensions)} 个三级小标题，且在每个标题下写出独立的分段分析，严禁合并，每个小标题下字数均不得少于100字】：\n{dimensions_bullet_list}")
    
    if weights_info:
        report_parts.append(f"3. **算法底层加权解码**：结合上述各个维度的剖析，带入底层设定的各个权重系数，解释系统是如何利用权重抑制次要矛盾、放大主要趋势的。")
    
    if is_wheeling:
        report_parts.append(f"4. **多维共振与特别号定胆 ({pre_sel_special}号)**：【⚠️重要纪律要求：必须在此总结章节，对上述全部勾选维度进行融合归纳，严密扣合最终敲定的特别号 {pre_sel_special}，论证其数学期望与图形均值回归的必然性】。经过对【{conv_text}】的多维独立分析和权重总结归票，论证在此多重约束下产生的合力是如何完美指向特别号 {pre_sel_special} 这个推算解的。")
        ...
        report_parts.append(f"4. **多维共振与最后定胆 ({pre_sel_special}号)**：【⚠️重要纪律要求：必须在此总结章节，对上述全部勾选维度进行融合归纳，严密扣合最终敲定的特码 {pre_sel_special}，论证其数学期望与图形均值回归的必然性】。经过对【{conv_text}】的多维独立分析和权重总结归票，论证在此多重约束下产生的合力是如何完美指向 {pre_sel_special} 这个推算解的。")
    ...
    if is_wheeling:
        ...
        target_lock_str = f"1. 目标锁定：你最后的分析结论【必须、无可争议地】指向特别号 {pre_sel_special}，逻辑演绎必须闭环于此号码。所有选中的图表规律和推算结论必须收敛并指向论证该号码的合理性与数学期望。任何与这组预测号码背道而驰的推理均为严重错误！精选码池正码 {pre_sel_nums} 在此作为旋转矩阵备选码池进行论证即可。"
    else:
        prompt_nums_str = f"👉 **本期底层算法最终敲定的【正码配置】：{pre_sel_nums}**"
        target_lock_str = f"1. 目标锁定：你最后的分析结论【必须、无可争议地】指向特码 {pre_sel_special}，逻辑演绎必须闭环于此号码。所有选中的图表规律和推算结论必须收敛并指向论证该号码的合理性与数学期望。任何与这组预测号码背道而驰的推理均为严重错误！正码 {pre_sel_nums} 在此作为陪衬提及即可。"
...
4. ✨ 深度关注图像反转与图表多模态综合（极核心）：你必须假装你正盯着几张实时更新的【数据图表】。请用“从最新的走势图、路单连线趋势、大小单双柱状图联动来看”等典型的图表视觉描述手法，把【单双】、【大小】、【尾数】和【路单连涨防跌】这几个图表维度的几何特征联合研判！提取它们在“图表轨迹”上的共振图形缩影（如双向探顶、W底、断崖式极限回踩等），通过大量描绘“图形盘面表征”，向用户论证均值回归规律必然爆发的原因。
