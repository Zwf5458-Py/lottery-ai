...
def init_auth_db():
    ...
    try:
        ...
        pass
    try:
        conn.execute("ALTER TABLE users ADD COLUMN reward_recharge_given INTEGER DEFAULT 0")
    except Exception:
        pass
    try:
...
def _generate_referral_code(conn) -> str:
    ...
    while True:
        ...
        row = conn.execute("SELECT 1 FROM users WHERE referral_code=?", (code,)).fetchone()
        if not row:
            return code
...
def register_user(username: str, password: str, ref_code: str = '') -> dict:
    ...
    try:
        ...
        referrer_id = None
        if ref_code:
            row = conn.execute("SELECT id FROM users WHERE referral_code=?", (ref_code.strip(),)).fetchone()
            if row:
                referrer_id = row[0]
...
def login_user(username: str, password: str) -> dict:
    ...
    try:
        ...
        # 兼容迁移：旧哈希在用户登录成功后自动升级为 PBKDF2
        if stored_hash and not stored_hash.startswith('pbkdf2_sha256$'):
            try:
                conn = sqlite3.connect(USERS_DB_PATH)
                conn.execute('PRAGMA journal_mode=WAL;')
                conn.execute("UPDATE users SET password_hash=? WHERE id=?", (_hash_password(password), row[0]))
                conn.commit()
                conn.close()
            except Exception:
                pass
...
def set_user_role(user_id: int, new_role: str) -> dict:
    ...
    try:
        conn = sqlite3.connect(USERS_DB_PATH)
        conn.execute('PRAGMA journal_mode=WAL;')
        prev_row = conn.execute("SELECT role FROM users WHERE id=?", (user_id,)).fetchone()
        if not prev_row:
            conn.close()
            return {'success': False, 'error': '用户不存在'}
        ...
        # 触发分享奖励（被推荐用户成为 VIP）
        reward_result = {'success': True, 'reward_given': False, 'reason': 'not_applicable'}
        try:
            if new_role == 'vip' and previous_role != 'vip':
                from modules.points_manager import apply_referral_vip_reward, calc_referral_reward_points
                from modules.config_manager import get_system_config
                sys_cfg = get_system_config()
                ...
                reward_points = calc_referral_reward_points(vip_points_base, reward_ratio, reward_points_fixed)
                reward_result = apply_referral_vip_reward(user_id, reward_points)
            elif new_role == 'vip' and previous_role == 'vip':
                reward_result = {'success': True, 'reward_given': False, 'reason': 'already_vip'}
        except Exception:
            reward_result = {'success': False, 'reward_given': False, 'reason': 'reward_exception'}
        return {
            'success': True,
